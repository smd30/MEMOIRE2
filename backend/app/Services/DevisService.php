...
class DevisService
{
    ...
    // Accessoires de police (FCFA) - Montant fixe
    private const ACCESSOIRES_POLICE_FIXE = 2000;
    
    // Tarif par défaut des garanties non tarifées (FCFA)
    private const TARIF_GARANTIE_DEFAUT = 100;

    // Taux de taxes
    private const TAUX_TUCA = 0.19; // 19%
    
    // Taux de réduction RC
    private const TAUX_REDUCTION_RC = 0.20; // 20%
    ...
    public function calculerGarantiesOptionnelles(array $garanties): float
    {
        ...
        foreach ($garanties as $garantie => $active) {
            if ($active) {
                // Si la garantie a un tarif défini, utiliser ce tarif
                if (isset(self::TARIFS_GARANTIES[$garantie])) {
                    $total += self::TARIFS_GARANTIES[$garantie];
                } else {
                    // Sinon, appliquer le tarif par défaut
                    $total += self::TARIF_GARANTIE_DEFAUT;
                }
            }
        }
    ...
    /**
     * Calcule les accessoires de police (montant fixe)
     */
    public function calculerAccessoiresPolice(array $accessoires = []): float
    {
        // Montant fixe de 2000 FCFA
        return self::ACCESSOIRES_POLICE_FIXE;
    }
    ...
    public function calculerDevis(array $donnees): array
    {
        ...
        // Prime RC annuelle brute
        $primeRCBruteAnnuelle = $this->calculerPrimeRC(
            $donnees['categorie'], 
            $donnees['caracteristiques']
        );
        
        // Application de la réduction de 20% sur la prime RC
        $primeRCAnnuelle = $primeRCBruteAnnuelle * (1 - self::TAUX_REDUCTION_RC);
        
        // Prime RC pour la période sélectionnée
        $primeRC = ($primeRCAnnuelle / 12) * $periodeMois;

        // Garanties optionnelles (fixes, pas proportionnelles)
        $garantiesOptionnelles = $this->calculerGarantiesOptionnelles(
            $donnees['garanties'] ?? []
        );
        
        // Garanties optionnelles annuelles (pour référence)
        $garantiesOptionnellesAnnuelle = $garantiesOptionnelles;
        ...
        return [
            ...
            'prime_rc_brute_annuelle' => $primeRCBruteAnnuelle,
            ...
            'reduction_rc' => self::TAUX_REDUCTION_RC * 100 . '%',
            'details' => [
